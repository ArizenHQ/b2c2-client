...
describe 'B2C2Client' do
  ...
  context 'Post' do
    ...
    context 'Trade' do
      ...
      let(:query_parameters)  {{
                                rfq_id: 'd779f888-d2e6-499c-92f1-d62c4c85f81b',
                                quantity: 5,
                                side: 'buy',
                                instrument: 'BTCUSD.SPOT',
                                price: '3999.99000000'
      ...
      it 'Has the good JSON query parameters' do
        expect(@private_client.trade(query_parameters).parameters).to eq(query_parameters)
      end

      it 'Returns the good JSON response' do
        expect(@private_client.trade(query_parameters).perform).to eq(response)
      end
    end
  end
